...
if action == 'True':
  ...
  rp_i, a_i, b_i, u1_i  = theta[0], theta[1], theta[2], theta[3] 
else:
  ...
  u2_i = 0 #0.1144 # Limb Darkening coefficient 2
...
def lnlike(theta, x, y, sigma, per=period):
  r, a, b, u1 = theta
  ...
  params.rp = r
  params.a = a
  params.inc = np.arccos(b_i/a_i)*(180./np.pi)
  params.ecc = 0
  params.w = 96
  params.u = [u1, 0]
...
initial = np.array([rp_i, a_i, b_i, u1_i]) + 1e-5*np.random.randn(ndim)
soln = minimize(nll, initial, args=(time, flux, sigma))
rp_ml, a_ml, b_ml, u1_ml = soln.x
...
final_fig.savefig(save_to + '/MCMCfit_opt.png', bbox_inches='tight')
...
def lightcurve_model(x, r, a, b, u1):
 # Set up transit parameters.
 params = batman.TransitParams()
 params.t0 = 0
 params.per = period
 params.rp = r
 params.a = a
 params.inc = np.abs(np.arccos(b/a))
 params.ecc = 0
 params.w = 96
 params.u = [u1, 0]
 params.limb_dark = 'quadratic'
...
initial_params = rp_i, a_i, b_i, u1_i
...
print('run time: {:.2f}'.format((t1-t0)/60) + 'min')
...
print('inclination??? ', np.arccos(theta_max[2] / theta_max[1]) * (180. / np.pi))
 
 

param_names = ["$rp$", "$a$", "$b$", "u1"]
